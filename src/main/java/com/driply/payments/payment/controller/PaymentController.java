...
public class PaymentController {
     ...
     * @param request 결제 요청에 관한 메타 데이터를 전달 받습니다.
     * @param requestDTO 결제사별로 api 요청에 필요한 데이터를 담고 있습니다.
    ...
    public ResponseEntity<Map<String, Object>> confirmPayment(HttpServletRequest request, @RequestBody PaymentRequestDTO requestDTO) throws IOException {
        Map<String, Object> response = paymentService.processPayment(request.getRequestURI(), requestDTO);
        return ResponseEntity.status(response.containsKey("error") ? 400 : 200).body(response);
    }

    /**
     * 정기결제를 수행합니다.
     * @param requestDTO 정기결제 요청에 필요한 데이터를 포함합니다.
     * @return 정기결제 성공 여부에 따라 200(정기결제 성공) 혹은 400(정기결제 실패) status code를 포함한 응답을 반환합니다.
     * @throws IOException
     */
    @PostMapping(value = "/confirm-billing")
    public ResponseEntity<Map<String, Object>> confirmBilling(@RequestBody BillingRequestDTO requestDTO) throws IOException {
        Map<String, Object> response = paymentService.confirmBilling(requestDTO);
        return ResponseEntity.status(response.containsKey("error") ? 400 : 200).body(response);
    }

    /**
     * 빌링키를 발급합니다. PG사 api로 빌링키 발급 요청을 처리합니다.
     * @param requestDTO 빌링키 발급에 필요한 데이터를 포함합니다.
     * @return 빌링키 발급 성공 여부에 따라 200(빌링키 발급 성공) 혹은 400(빌링키 발급 실패) status code를 포함한 응답을 반환합니다.
     * @throws IOException
     */
    @PostMapping(value = "/issue-billing-key")
    public ResponseEntity<Map<String, Object>> issueBillingKey(@RequestBody BillingKeyRequestDTO requestDTO) throws IOException {
        Map<String, Object> response = paymentService.issueBillingKey(requestDTO);
        return ResponseEntity.status(response.containsKey("error") ? 400 : 200).body(response);
    }
     ...
     * @param requestDTO 결제 승인 요청에 필요한 데이터를 포함합니다.
    ...
    public ResponseEntity<Map<String, Object>> confirmBrandpay(@RequestBody BrandpayRequestDTO requestDTO) throws IOException {
        Map<String, Object> response = paymentService.confirmBrandpay(requestDTO);
        return ResponseEntity.status(response.containsKey("error") ? 400 : 200).body(response);
    }
